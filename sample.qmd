...
`r nrow(survey_results)` respondents finished the survey, broken out by affiliation and role below:

```{r}
plotly_treemap_df <- survey_results |>  
...
plotly_treemap_df <- plotly_treemap_df |> 
  bind_rows(
    survey_results |> 
      group_by(type = QID8) |> 
      summarise(n = n()) |> 
