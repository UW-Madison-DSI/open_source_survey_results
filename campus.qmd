...
**`r round(length(survey_results$QID24[survey_results$QID24 %in% c('Very valuable', 'Some value')]) / nrow(survey_results), 2) * 100`%** of respondents agreed that there's at least some value in a vibrant campus open source culture.
...
c1_df |>
  ...
  layout(
    ...
    title = paste0('"Having a vibrant open source culture at ', uni_name, ' is important"'),
    xaxis = list(title =  ''),
    yaxis = list(
      zerolinecolor = '#ffff',
      zerolinewidth = 2,
...
In comparison, **`r round(length(survey_results$QID23[survey_results$QID23 %in% c('Strongly agree', 'Somewhat agree')]) / nrow(survey_results), 2) * 100`%** respondents agreed that there is a vibrant open source culture at `r uni_name`.
